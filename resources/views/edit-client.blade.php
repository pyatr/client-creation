...
<div id="data-main-content" class="main-content">
    ...
    <form id="creation-form" method="post">
        ...
        <input type="hidden" id="oldEmail" name="oldEmail"><br>
...
<script>
    ...
    fetch("http://" + window.location.host + "/clients/specific?" + params).then(function (response) {
        return response.json();
    }).then(function (apiJsonData) {
        if (apiJsonData.length > 0) {
            Object.keys(apiJsonData[0]).forEach(elementID => {
                let element = document.getElementById(elementID);
                if (element != null) {
                    element.value = apiJsonData[0][elementID];
                }
            });
            document.getElementById("oldEmail").value = apiJsonData[0]['email'];
        }
    });
